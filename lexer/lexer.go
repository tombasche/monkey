...
func (l *Lexer) makeTwoCharToken(next byte, twoChar token.TokenType, single token.TokenType) token.Token {
	if l.peekChar() == next {
		ch := l.ch
		l.readChar()
	...
	return newToken(single, l.ch)
}

func (l *Lexer) readIdentifier(method func(byte) bool) string {
	position := l.position
	for method(l.ch) {
		l.readChar()
	}
